...
/* ============================================================================
   ANALYTICS DASHBOARD — Chart.js line/bar charts + summary stats
   ============================================================================ */
...
window.renderAnalyticsDashboard = function() {
    ...
    if (typeof Chart === 'undefined') {
        bodyEl.innerHTML = `<div class="report-empty">Charts couldn't load (no network access to fetch Chart.js). Everything else in the app still works offline.</div>`;
        return;
    }
    ...
    const allTimes = history.flatMap(h => (h.questionBreakdown || []).map(q => q.timeSpentSec).filter(t => typeof t === 'number'));
    ...
    document.getElementById('analytics-summary-grid').innerHTML = `
        <div class="report-stat-card"><div class="val">${history.length}</div><div class="lbl">Quizzes Taken</div></div>
        <div class="report-stat-card"><div class="val">${overallAcc}%</div><div class="lbl">Overall Accuracy</div></div>
        <div class="report-stat-card"><div class="val">${avgTime}s</div><div class="lbl">Avg Time / Question</div></div>
        <div class="report-stat-card"><div class="val">${totalQuestions}</div><div class="lbl">Questions Answered</div></div>
    `;

    const gridColor = themeColor('--card-border', 'rgba(255,255,255,0.1)');
    const textColor = themeColor('--text', '#e0e0ff');
    const primary = themeColor('--primary', '#00f2ff');
    const success = themeColor('--success', '#00ff88');
    const warning = themeColor('--warning', '#ffc107');
    const danger = themeColor('--danger', '#ff0055');
    Chart.defaults.color = textColor;
    Chart.defaults.font.family = getComputedStyle(document.body).fontFamily;

    // ---- Line chart: score trend over recent attempts (oldest -> newest, left to right) ----
    const recent = history.slice(0, 20).reverse();
    const trendLabels = recent.map((h, i) => `#${history.length - recent.length + i + 1}`);
    const trendData = recent.map(h => h.total > 0 ? Math.round((h.score / h.total) * 100) : 0);

    destroyChart('trend');
    _chartInstances.trend = new Chart(document.getElementById('chart-score-trend'), {
        type: 'line',
        data: {
            labels: trendLabels,
            datasets: [{
    ...
    history.forEach(h => {
        ...
        topicStats[key].correct += h.score;
        topicStats[key].total += h.total;
    });
    const topicEntries = Object.entries(topicStats).sort((a, b) => b[1].total - a[1].total).slice(0, 10);
    const topicLabels = topicEntries.map(([t]) => (t.length > 18 ? t.slice(0, 16) + '…' : t));
    const topicAcc = topicEntries.map(([, d]) => d.total > 0 ? Math.round((d.correct / d.total) * 100) : 0);
    const topicColors = topicAcc.map(pct => pct >= 80 ? success : pct >= 50 ? warning : danger);

    destroyChart('topic');
    _chartInstances.topic = new Chart(document.getElementById('chart-topic-accuracy'), {
        type: 'bar',
        data: { labels: topicLabels, datasets: [{ label: 'Accuracy %', data: topicAcc, backgroundColor: topicColors, borderRadius: 6 }] },
        options: {
            responsive: true,
            plugins: { legend: { display: false } },
            scales: {
    ...
    // ---- Bar chart: avg time per question by topic ----
    const timeStats = {};
    history.forEach(h => {
        const key = h.topic || 'Unknown';
        (h.questionBreakdown || []).forEach(q => {
            if (typeof q.timeSpentSec !== 'number') return;
            if (!timeStats[key]) timeStats[key] = { sum: 0, n: 0 };
            timeStats[key].sum += q.timeSpentSec;
            timeStats[key].n += 1;
        });
    });
    const timeEntries = Object.entries(timeStats).filter(([, d]) => d.n > 0).sort((a, b) => b[1].n - a[1].n).slice(0, 10);

    destroyChart('time');
    if (timeEntries.length === 0) {
        document.getElementById('chart-time-per-question').parentElement.innerHTML = `<div class="report-empty">No per-question timing data yet — take a few more quizzes to see this.</div>`;
    } else {
        const timeLabels = timeEntries.map(([t]) => (t.length > 18 ? t.slice(0, 16) + '…' : t));
        const timeAvgs = timeEntries.map(([, d]) => Math.round(d.sum / d.n));
        _chartInstances.time = new Chart(document.getElementById('chart-time-per-question'), {
            type: 'bar',
            data: { labels: timeLabels, datasets: [{ label: 'Avg seconds', data: timeAvgs, backgroundColor: primary + 'aa', borderRadius: 6 }] },
            options: {
                responsive: true,
                plugins: { legend: { display: false } },
                scales: {
